...
| Visual identity | Distinct ship silhouettes by strength, fictional letter coins mounted on enemy hulls, gold mathematical π coin on player ship | Twenty supplied top-down designs prepared as transparent atlas; class-based silhouettes and four selectable player hulls/four paints integrated; live visual review pending |
...
The supplied 20-ship reference is preserved at `doc/assets/reference-fleet.jpeg`; a transparent derivative is used at `frontend/public/ships/cryptoid-fleet.png`. Enemy classes choose distinct silhouettes from the sheet and mount a smaller fictional letter coin. The former detached health bar is now a ring on that coin. The player can select four hull silhouettes and four paints in the hangar; selection is stored locally for subsequent runs. All four are available as a prototype. A future Crypto Shard inventory and unlock system can gate extra cosmetic hulls/paints, with preview, prices, saved ownership and no gameplay advantage. The existing Pi Shop payment products do not sell these skins.
...
1. **Contact fairness audit.** The current code applies `shipHitsEnemy` only to enemies actively executing an attack, then passes the impact through `receiveImpacts`; a shield charge absorbs it, otherwise one heart is lost. The existing 1.5-second impact cooldown prevents immediate repeated hits. Check this end-to-end with one life lost on an unshielded attack collision, one shield charge consumed when protected, no lost life on a near miss, no accidental collision damage in the non-attacking bonus phase, visible hit feedback and a working Game Over/restart. If persistent overlap can cause a second life loss after cooldown, add separation or one-contact-per-attack behavior. Confirm enemy-shot impacts use the same rules.
...
3. **Living starfield.** Replace the current tiled CSS star pattern in `.star-layer` with a deterministic irregular distribution across the whole viewport: mixed sizes, brightness and colors with no obvious repeated rows or grid. Remove or greatly subdue the separate `.game-field::before` grid wherever a natural starfield is shown. Add a subdued Milky Way band/nebula in selected sectors and slight depth-dependent drift or twinkle. Rare shooting stars traverse visible, varied paths, fade quickly and never resemble hostile projectiles. Bound their count and animation cost; pause with gameplay, respect reduced-motion preferences and keep HUD, ships, pickups and projectiles readable.
