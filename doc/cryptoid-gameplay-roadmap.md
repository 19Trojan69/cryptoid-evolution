...
- Every individual level must advance a small internal difficulty value so the player can notice a minimal, fair progression from the beginning. Use continuous weights and fractional tuning for attack pauses, formation occupancy chance, pattern selection, projectile pressure and enemy mixture rather than forcing an extra whole enemy or extra health point on every level.
- Every block of 20 levels adds one clearer milestone or unlock. This creates 25 gentle milestone stages through level 500 while the underlying curve still moves slightly on every level.
- Each milestone changes only a small part of the challenge at a time: formation occupancy, attack-group chance, projectile budget, enemy mixture, elite chance, durability, or a tightly capped speed increase.
- Add automated checks that the per-level difficulty score never decreases, never exceeds the defined safety caps and does not create a large jump at a 20-level boundary.
...
- Owner proposal for later evaluation: recurring monthly Pi payments, potentially implemented with smart contracts if the Pi platform officially supports the required recurring-payment flow when development begins. Do not assume automatic debits or subscription contracts are available; verify the current official capabilities first.
- Owner proposal for later evaluation: if monthly payment stops, continued cloud-save access ends and the remote run might eventually be deleted. This is not yet approved for implementation. First define explicit consent, advance notices, a reasonable grace period, reactivation, export/local fallback and treatment of temporary payment or platform failures.
- Preferred fair retention rule: freeze the optional cloud checkpoint as read-only during a grace period instead of immediately deleting it. Never delete owned ships, paid products, Shards, records or a valid local save merely because a cloud subscription ends. Permanent remote deletion should occur only under a clearly disclosed retention rule or an explicit player request.
