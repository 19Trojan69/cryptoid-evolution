...
const cryptoidMotionClass = (asteroid: Asteroid) => {
  if (asteroid.returnElapsed > 0) return " cryptoid-reverse";
  ...
  return (
    <main className="game-shell" onPointerDownCapture={() => { void startEffects(); requestGameFullscreen(); }}>
      <div ref={fieldRef} className="game-field" onPointerDown={startDrag} onPointerMove={event => { if (pointerRef.current === event.pointerId) positionFromPointer(event); }} onPointerUp={event => { if (pointerRef.current === event.pointerId) pointerRef.current = null; }} onPointerCancel={event => { if (pointerRef.current === event.pointerId) pointerRef.current = null; }}>
        ...
        {game.encounter === "normal" && !isBonusSection(game.section) && (game.phase === "ENTRY" || game.phase === "FORMATION" || game.phase === "REFORM") && game.asteroids.map(asteroid => { const locked = asteroid.entryElapsed >= asteroid.entryDuration && asteroid.formationElapsed >= asteroid.formationDuration; const sprite = enemySprite(asteroid.shipClass, asteroid.formationSlot); const offset = spriteVisualOffset(sprite, asteroid.radius * 2, true); return <div key={`formation-${asteroid.id}`} className={`formation-target${locked ? " formation-target-locked" : ""}`} style={{ left: asteroid.entryTargetX + offset.x, top: asteroid.entryTargetY + offset.y, width: asteroid.radius * 1.65, height: asteroid.radius * 1.65 }} aria-hidden="true"><span /></div>; })}
        ...
        <div className="touch-controls">
          <div className="edge-actions" role="group" aria-label={t('Available equipment')}>
            ...
            {game.pickupWeaponMs > 0 && <span className="edge-action edge-action-pickup" title={`${t("Collected shots")} · ${Math.ceil(game.pickupWeaponMs / 1_000)}s`} aria-label={`${t("Collected shots")} ${Math.ceil(game.pickupWeaponMs / 1_000)} ${t("seconds remaining")}`}>↑<small>{Math.ceil(game.pickupWeaponMs / 1_000)}s</small></span>}
            ...
            {game.rapidFireMs > 0 && <span className="edge-action edge-action-rapid" aria-label={`Rapid Fire ${Math.ceil(game.rapidFireMs / 1_000)} ${t("seconds remaining")}`}>»<small>{Math.ceil(game.rapidFireMs / 1_000)}s</small></span>}
            {game.overdriveMs > 0 && <span className="edge-action edge-action-overdrive" aria-label={`Overdrive ${Math.ceil(game.overdriveMs / 1_000)} ${t("seconds remaining")}`}>ϟ<small>{Math.ceil(game.overdriveMs / 1_000)}s</small></span>}
          </div>
        </div>
