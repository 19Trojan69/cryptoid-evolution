...
import { enemySprite, selectedShip, shardBalance, SHARD_BALANCE_KEY, shipHullStyle, shipNozzleStyles, spriteStyle, spriteVisualOffset } from "./shipFleet";
...
const engineTrails = (sprite: number, className: "exhaust" | "player-engine", facesPlayer = false) =>
  shipNozzleStyles(sprite, facesPlayer).map((style, index) => <span key={`${className}-${index}`} className={className} style={style} />);
  ...
  return (
    <main className="game-shell" onPointerDownCapture={() => { void startEffects(); requestGameFullscreen(); }}>
      <div ref={fieldRef} className="game-field" onPointerDown={startDrag} onPointerMove={event => { if (pointerRef.current === event.pointerId) positionFromPointer(event); }} onPointerUp={event => { if (pointerRef.current === event.pointerId) pointerRef.current = null; }} onPointerCancel={event => { if (pointerRef.current === event.pointerId) pointerRef.current = null; }}>
        ...
        <div ref={playerShipRef} className={`player-ship${shipSelection.color.id === "grey" || shipSelection.color.id === "white" ? ` player-ship-${shipSelection.color.id}` : ""}${game.shieldActive && game.shieldCharges > 0 && game.shieldMs > 0 ? " player-ship-shield-active" : ""}${game.effects.some(effect => effect.target === "player" && effect.kind === "hit") ? " player-ship-hurt" : ""}${game.effects.some(effect => effect.target === "player" && effect.kind === "shield") ? " player-ship-shielded" : ""}`} style={{ left: `${game.player.x * 100}%`, top: `${game.player.y * 100}%`, "--ship-glow": shipSelection.color.glow, "--flame-length": `${5 + game.thrust * 13}%` } as CSSProperties} aria-label={t('Your Cryptoid ship')}><PaintedShip className="fleet-sprite" sprite={shipSelection.skin.sprite} color={shipSelection.color.id} />{engineTrails(shipSelection.skin.sprite, "player-engine")}</div>
        ...
        <div className="touch-controls">
          <div className="edge-actions" role="group" aria-label={t('Available equipment')}>
            ...
            {game.shieldCharges > 0 && <button type="button" className="edge-action edge-action-shield" aria-pressed={game.shieldActive} aria-label={`${t(game.shieldActive ? "Disable" : "Enable")} ${t("shield")} · ${game.shieldCharges} ${t("charge")}`} title={`${t("Shield")} · ${game.shieldCharges} ${t("charge")} · ${Math.ceil(game.shieldMs / 1_000)}s`} onClick={() => { stateRef.current.shieldActive = !stateRef.current.shieldActive; setGame({ ...stateRef.current }); }}>{powerUpSymbols.shield}<small>{Math.ceil(game.shieldMs / 1_000)}s</small></button>}
