...
export type SectorBoss = { x: number; y: number; radius: number; health: number; maxHealth: number; elapsed: number; fireElapsed: number };

export const createSectorBoss = (sector: number, width: number): SectorBoss => {
  const health = Math.min(36, 12 + sector * 3);
  return { x: width / 2, y: -60, radius: 50, health, maxHealth: health, elapsed: 0, fireElapsed: 0 };
};
...
export const moveSectorBoss = (boss: SectorBoss, delta: number, width: number, height: number): SectorBoss => {
  ...
  return {
    ...
    y: -boss.radius + (height * .24 + boss.radius) * (entry * entry * (3 - 2 * entry)),
    elapsed,
